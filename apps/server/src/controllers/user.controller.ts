...
export function createUserController(app: FastifyInstance) {
  ...
  async function updateMe(
    ...
    if (name !== undefined && name.trim().length === 0) {
      return reply
        .code(400)
        .send(badRequest(t(request.lang, MSG.AUTH_EMAIL_REQUIRED)));
    }

    if (status !== undefined && !USER_STATUSES.includes(status)) {
      return reply
        .code(400)
        .send(badRequest(t(request.lang, MSG.VALIDATION_INVALID_STATUS)));
    }
  ...
  async function uploadAvatar(request: FastifyRequest, reply: FastifyReply) {
    ...
    if (!data) {
      return reply
        .code(400)
        .send(badRequest(t(request.lang, MSG.USER_AVATAR_REQUIRED)));
    }
    ...
    if (!allowed.includes(data.mimetype)) {
      return reply
        .code(400)
        .send(badRequest(t(request.lang, MSG.USER_AVATAR_INVALID)));
    }
