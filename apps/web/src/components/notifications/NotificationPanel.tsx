  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      <div className="flex items-center gap-2 border-b border-stone-100 px-4 py-3">
        ...
        <span className="flex-1 text-sm font-semibold text-stone-800">{t("detailTitle")}</span>
        <button
          type="button"
          onClick={onClose}
          className="flex h-7 w-7 items-center justify-center rounded-lg text-stone-400 hover:bg-stone-100 hover:text-stone-600"
      ...
      <div className="flex items-start gap-3 border-b border-stone-50 px-5 py-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <h2 className="mt-1 text-sm font-semibold leading-snug text-stone-800">{notification.title}</h2>
        </div>
      </div>
      ...
      <ul className="flex-1 overflow-y-auto divide-y divide-stone-50">
        {notifications.length === 0 ? (
          ...
          notifications.map((n) => {
            ...
            const shortMsg = rawMsg ? (rawMsg.length > 50 ? rawMsg.slice(0, 50) + "…" : rawMsg) : null;
            return (
              <li
                key={n.id}
...
export function NotificationPanel() {
  ...
  const { notifications, markAsRead, markAllAsRead, panelOpen, closePanel } = useNotificationContext();
