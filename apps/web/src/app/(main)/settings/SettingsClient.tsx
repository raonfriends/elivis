...
function AvatarUpload({ user }: { user: UserProfile | null }) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const maxSize = (Number(process.env.NEXT_PUBLIC_UPLOAD_MAX_FILE_SIZE_MB) || 2) * 1024 * 1024;
  ...
  return (
    <div className="flex flex-col items-center gap-3">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-0.5 rounded-full bg-black/40 opacity-0 transition-opacity group-hover:opacity-100">
            {isPending ? (
              ...
              <>
                ...
                <span className="text-[10px] font-medium text-white">{t("avatarChange")}</span>
              </>
            )}
          </div>
        </button>
      ...
      {error && (
        <p className="max-w-[200px] text-center text-xs text-red-500">{error}</p>
      )}
    </div>
  );
}
...
function ProfileTab({ user }: { user: UserProfile | null }) {
  ...
  return (
    <div className="space-y-8">
      {/* 프로필 사진 */}
      <div>
        <p className="mb-4 text-sm font-medium text-stone-700">{t("profile.avatarLabel")}</p>
      ...
      {/* 기본 정보 폼 */}
      <div>
        <h2 className="mb-5 text-base font-semibold text-stone-800">{t("profile.sectionTitle")}</h2>
